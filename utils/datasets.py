...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        ...
        for i, s in enumerate(sources):
            ...
            cap = cv2.VideoCapture(url, cv2.CAP_FFMPEG) #linne + cv2.CAP_FFMPEG
            cap.set(cv2.CAP_PROP_BUFFERSIZE, 2) #linne test
            assert cap.isOpened(), f'Failed to open {s}'
            w = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
            h = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
            # self.fps_f = 1/25 #linne test
            # self.fps = (self.fps_f *1000)#linne test
    ...
    def update(self, index, cap):
        ...
        while cap.isOpened():
            ...
            if n == 1:  # read every frame? ;linne test
...
def letterbox(img, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
    ...
    dh = torch.tensor(dh) #linne addition
    dw = torch.tensor(dw) #linne addition
...
def box_candidates(box1, box2, wh_thr=2, ar_thr=10, area_thr=0.1, eps=1e-16):  # box1(4,n), box2(4,n), Originally ar_thr=20
